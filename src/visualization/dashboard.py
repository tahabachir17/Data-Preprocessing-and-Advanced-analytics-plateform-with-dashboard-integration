...
class Dashboard:
    ...
    def render_column_selector(
        self, df: pd.DataFrame, key_prefix: str = ""
        ...
        with col2:
            ...
            elif (len(numeric_cols) == 1 and numeric_cols[0] != x_col) or (numeric_cols and x_col not in numeric_cols):
    ...
    def render_plot_customization(self, key_prefix: str = "") -> Dict:
        ...
        with st.expander("🎛️ Plot Customization", expanded=False):
            ...
            with col1:
                title = st.text_input(
                    "Plot Title:", placeholder="Enter custom title (optional)", key=f"{key_prefix}_title"
                )

                width = st.slider(
                    "Plot Width (px):", min_value=600, max_value=1600, value=1200, step=100, key=f"{key_prefix}_width"
                )

            with col2:
                height = st.slider(
                    "Plot Height (px):", min_value=400, max_value=1000, value=600, step=50, key=f"{key_prefix}_height"
                )
        ...
        return {"title": title if title else None, "width": width, "height": height, "template": template}
    ...
    def render_data_filter(self, df: pd.DataFrame, key_prefix: str = "") -> pd.DataFrame:
        ...
        with st.expander("🔍 Data Filtering", expanded=False):
            ...
            filter_col = st.selectbox(
                "Filter by column:", ["None"] + df.columns.tolist(), key=f"{key_prefix}_filter_col"
            )
                ...
                if plot_idx < len(plots_history):
                    ...
                    with col:
                        ...
                        if "figure" in plot_info:
                            st.plotly_chart(
                                plot_info["figure"], use_container_width=True, key=f"gallery_plot_{plot_idx}"
                            )
    ...
    def create_comprehensive_dashboard(self, df: pd.DataFrame, key_prefix: str = "main") -> Optional[go.Figure]:
        ...
        try:
            with st.spinner(f"Creating {plot_type.lower()}..."):
                ...
                fig.update_layout(
                    template=custom_options["template"], height=custom_options["height"], width=custom_options["width"]
                )
            ...
            st.error(f"Error creating plot: {e!s}")
    ...
    def create_multi_variable_dashboard(self, df: pd.DataFrame):
        ...
        if target_col:
            if st.button("Show Target Correlation Matrix"):
                try:
                    ...
                    st.error(f"Error generating target correlation matrix: {e!s}")
        ...
        if len(numeric_df.columns) >= 2:
            ...
            selected_vars = st.multiselect(
                ...
                default=(
                    numeric_df.columns.tolist()[:4] if len(numeric_df.columns) >= 4 else numeric_df.columns.tolist()
                ),
            )
                    ...
                    for i, var1 in enumerate(selected_vars):
                        for j, var2 in enumerate(selected_vars):
                            if i == j:
                                # Diagonal: histogram
                                fig.add_trace(
                                    go.Histogram(x=df[var1], name=var1, showlegend=False), row=i + 1, col=j + 1
                                )
                            else:
    ...
    def render_plot_insights(self, df: pd.DataFrame, x_col: str, y_col: str = None):
        ...
        with st.expander("🧠 Automated Insights", expanded=False):
            ...
            if pd.api.types.is_numeric_dtype(df[x_col]):
                ...
                insights.append(
                    f"📈 {x_col} ranges from **{df[x_col].min():.2f}** to **{df[x_col].max():.2f}** (range: {x_range:.2f})"
                )
            ...
            if y_col and pd.api.types.is_numeric_dtype(df[y_col]):
                ...
                if correlation is not None:
                    corr_strength = (
                        "strong" if abs(correlation) > 0.7 else "moderate" if abs(correlation) > 0.3 else "weak"
                    )
                    corr_direction = "positive" if correlation > 0 else "negative"
                    insights.append(
                        f"🔗 **{corr_strength.title()} {corr_direction}** correlation between {x_col} and {y_col} (r = {correlation:.3f})"
                    )
