...
class DataVisualizer:
    ...
    def _evenly_downsample(self, df: pd.DataFrame, max_points: int, sort_by: Optional[str] = None) -> pd.DataFrame:
    ...
    def _prepare_distribution_grouping(self, df: pd.DataFrame, x_col: str) -> Tuple[pd.DataFrame, str]:
        plot_df = df.copy()
    ...
    def create_scatter_plot(
        ...
        try:
            ...
            fig.update_layout(xaxis_title=x_col, yaxis_title=y_col, showlegend=bool(color_col), height=600)
    ...
    def create_bar_plot(
        ...
        try:
            ...
            fig = px.bar(
                agg_df, x=x_col, y=y_col, color=color_col, title=title or f"{y_col} by {x_col}", template="plotly_white"
            )
    ...
    def create_histogram(
        self, df: pd.DataFrame, x_col: str, color_col: Optional[str] = None, title: Optional[str] = None
    ) -> go.Figure:
        ...
        try:
            ...
            fig.update_layout(xaxis_title=x_col, yaxis_title="Count", showlegend=bool(color_col), height=600)
    ...
    def create_heatmap(
        self, df: pd.DataFrame, x_col: str, y_col: str, value_col: Optional[str] = None, title: Optional[str] = None
    ) -> go.Figure:
        ...
        try:
            ...
            fig = px.imshow(
                pivot_df, title=title or f"Heatmap of {x_col} vs {y_col}", template="plotly_white", aspect="auto"
            )

            fig.update_layout(xaxis_title=x_col, yaxis_title=y_col, height=600)
        ...
        try:
            ...
            if color_col and color_col in plot_df.columns:
                for category in plot_df[color_col].dropna().unique():
                    ...
                    fig.add_trace(
                        go.Histogram(x=subset[x_col], histnorm="probability density", name=str(category), opacity=0.7)
                    )
            else:
                fig.add_trace(go.Histogram(x=plot_df[x_col], histnorm="probability density", name=x_col, opacity=0.7))
    ...
    def create_bubble_plot(
        ...
        try:
            ...
            plot_df = self._evenly_downsample(
                df[plot_columns].dropna(subset=[x_col, y_col, size_col]), self.max_scatter_points
            )
            fig = px.scatter(
                plot_df,
                x=x_col,
