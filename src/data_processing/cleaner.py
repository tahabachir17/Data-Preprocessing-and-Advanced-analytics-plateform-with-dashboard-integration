...
class DataCleaner:
    ...
    def _merge_datetime_and_time_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["date_time"] = pd.to_datetime(date_part + " " + time_part, errors="coerce")
        ...
        total_count = len(df)
        print(
            f"   ✅ Merged '{date_col}' + '{time_col}' → 'date_time' "
        ...
        for col in df.columns:
            if pd.api.types.is_numeric_dtype(df[col]):
                ...
                if col_dtype == "object":
                    ...
                    if invalid_count > 0:
                        print(
                            f"   ⚠️ Column '{col}' has {invalid_count} invalid numeric-like string values. Setting to NaN."
                        )
    ...
    def _interpolate_by_row_frequency(self, df: pd.DataFrame, frequency_lines: int = 8) -> pd.DataFrame:
        ...
        for col in numeric_cols:
            ...
            print(
                f"   • Initial missing values: {initial_missing}/{total_rows} ({initial_missing/total_rows*100:.1f}%)"
            )
            ...
            for group_num in row_groups.unique():
                ...
                if len(valid_values) > 0:
                    ...
                    if filled_count > 0:
                        print(
                            f"     📦 Group {group_num}: Using {group_value:.3f} (from {len(valid_values)} valid values)"
                        )
                else:
                    group_value = 0
                    filled_count = group_data.isnull().sum()
        ...
        print("✅ ROW-BASED INTERPOLATION COMPLETED:")
    ...
    def _handle_missing_values(
        self, df: pd.DataFrame, data_type: str = "statique", frequency_lines: int = 8
    ) -> pd.DataFrame:
        """
        ...
        if data_type == "frequence":
            ...
            columns_to_drop = [
                col for col in df_cleaned.columns if any(keyword.lower() in col.lower() for keyword in keywords)
            ]
