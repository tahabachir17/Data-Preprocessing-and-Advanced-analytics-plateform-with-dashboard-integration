...
def remove_zero_only_rows(df: pd.DataFrame, datetime_col: str = None) -> pd.DataFrame:
    ...
    for col in df.columns:
        if pd.api.types.is_datetime64_any_dtype(df[col]) or col.lower() in {"datetime", "date", "time", "timestamp", "date_time", "created_at", "updated_at"}:
            cols_to_ignore.add(col)

    # Step 2: Select only numeric columns to check
    numeric_cols = [col for col in df.select_dtypes(include="number").columns if col not in cols_to_ignore]
...
class SmartCategoricalEncoder:
    ...
    def fit(self, X, y=None):
        ...
        self.categorical_columns_ = [col for col in frame.columns if col not in self.numeric_columns_]
...
class RegressionFeaturePreprocessor:
    ...
    def fit(self, X, y):
        ...
        self.categorical_features_ = [col for col in frame.columns if col not in self.numerical_features_]
        ...
        if self.numerical_features_:
            ...
            scaled[self.numerical_features_] = self.scaler.fit_transform(scaled[self.numerical_features_])

            correlations = scaled[self.numerical_features_].corrwith(y_series).abs().fillna(0)
            self.removed_features_ = correlations[correlations <= self.correlation_threshold].index.tolist()
            scaled = scaled.drop(columns=self.removed_features_, errors="ignore")
            self.numerical_features_ = [
                feature for feature in self.numerical_features_ if feature not in self.removed_features_
            ]

        active_categorical_features = [col for col in self.categorical_features_ if col in scaled.columns]
        encoders = {}
    ...
    def transform(self, X):
        ...
        if self.scaler is not None and self.scaler_feature_names_:
            ...
            scaled_df = pd.DataFrame(scaled_values, index=frame.index, columns=self.scaler_feature_names_)
            for column in self.numerical_features_:
...
class MLModels:
    ...
    def remove_zero_correlation_features(self, X, y, correlation_threshold=0.01):
        ...
        try:
            ...
            if features_to_remove:
                print(
                    f"Removing {len(features_to_remove)} numerical features with correlation <= {correlation_threshold}"
                )
    ...
    def prepare_data_pipeline(
        ...
        """
        print("Starting regression data preparation pipeline...")
    ...
    def get_regression_models(self):
        ...
        models = {
            ...
            "Lasso Regression": {
                ...
                "params": {"alpha": [0.1, 1.0, 10.0], "selection": ["cyclic", "random"]},
            },
            "Random Forest": {
    ...
    def generate_validation_curve_data(self, model_name, best_params, X, y, cv_folds=ML_CV_FOLDS):
        ...
        fixed_params = {key: value for key, value in (best_params or {}).items() if key != param_name}
        if fixed_params:
    ...
    def generate_iteration_history(self, model_name, model, X_train, y_train, X_test, y_test):
        ...
        train_rmse = [float(np.sqrt(mean_squared_error(y_train, prediction))) for prediction in train_predictions]
        validation_rmse = [float(np.sqrt(mean_squared_error(y_test, prediction))) for prediction in test_predictions]
    ...
    def compile_model_diagnostics(self, model_name, model_info, cv_folds=ML_CV_FOLDS):
        ...
        diagnostics = {
            "learning_curve": self.generate_learning_curve_data(model, self.X_train, self.y_train, cv_folds=cv_folds),
    ...
    def train_and_select_best_model(
        ...
        if selected_model_names:
            models = {name: config for name, config in models.items() if name in selected_model_names}
            ...
            try:
                ...
                if search_strategy == "random":
                    ...
                    model_search = GridSearchCV(
                        estimator=model, param_grid=params, cv=cv_folds, scoring="r2", n_jobs=self.n_jobs, verbose=1
                    )
                ...
                y_train_array = (
                    ...
                    else np.array(y_train, dtype=np.float64)
                )
                ...
                print(f"Error training {name}: {e!s}")
    ...
    def get_feature_importance(self, model=None, feature_names=None):
        ...
        try:
            ...
            if hasattr(model, "feature_importances_"):
                ...
                importance_df = pd.DataFrame(
                    {"feature": feature_names, "importance": importance_values.flatten()}
                ).sort_values("importance", ascending=False)
                return importance_df
            else:
                return None
    ...
    def full_regression_pipeline(
        ...
        try:
            ...
            test_metrics, test_predictions = self.evaluate_model(self.best_model, X_test, y_test)
            ...
            if raw_input_df is not None and categorical_preprocessor is not None:
                try:
                    ...
                    regression_preprocessor = RegressionFeaturePreprocessor(correlation_threshold=correlation_threshold)
                    ...
                    print("Warning: could not build the optional raw prediction pipeline: " f"{pipeline_error}")
            ...
            print(f"Error in regression pipeline: {e!s}")
            raise
