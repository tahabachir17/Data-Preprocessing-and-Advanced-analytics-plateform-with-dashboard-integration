...
class StatisticalAnalyzer:
    ...
    def test_normality(self, data: pd.Series, alpha: float = 0.05) -> Dict[str, Any]:
        ...
        results = {"sample_size": len(clean_data), "alpha": alpha, "tests": {}}
        ...
        try:
            ...
            results["tests"]["anderson_darling"] = {
                ...
                "is_normal": is_normal,
                "interpretation": (
                    "Data appears normally distributed" if is_normal else "Data does not appear normally distributed"
                ),
            }
        except Exception as e:
        ...
        # Overall conclusion
        normal_votes = sum(
            1 for test in results["tests"].values() if isinstance(test, dict) and test.get("is_normal", False)
        )
        total_tests = sum(1 for test in results["tests"].values() if isinstance(test, dict) and "is_normal" in test)
    ...
    def one_sample_ttest(
        self, data: pd.Series, population_mean: float, alpha: float = 0.05, alternative: str = "two-sided"
    ) -> Dict[str, Any]:
        """
    ...
    def paired_ttest(
        self, before: pd.Series, after: pd.Series, alpha: float = 0.05, alternative: str = "two-sided"
    ) -> Dict[str, Any]:
        """
    ...
    def one_way_anova(
        self, df: pd.DataFrame, value_column: str, group_column: str, alpha: float = 0.05
    ) -> Dict[str, Any]:
        """
        ...
        if p_value < alpha and len(groups) > 2:
            try:
                ...
                result["post_hoc"] = {"test": "Tukey HSD", "note": "Pairwise comparisons available"}
            except:
    ...
    def chi_square_independence(
        self, df: pd.DataFrame, column1: str, column2: str, alpha: float = 0.05
    ) -> Dict[str, Any]:
        """
    ...
    def chi_square_goodness_of_fit(
        self, observed: pd.Series, expected: Optional[List[float]] = None, alpha: float = 0.05
    ) -> Dict[str, Any]:
        """
    ...
    def mann_whitney_u(
        self, group1: pd.Series, group2: pd.Series, alpha: float = 0.05, alternative: str = "two-sided"
    ) -> Dict[str, Any]:
        """
    ...
    def wilcoxon_signed_rank(
        self, x: pd.Series, y: pd.Series = None, alpha: float = 0.05, alternative: str = "two-sided"
    ) -> Dict[str, Any]:
        """
    ...
    def kruskal_wallis(
        ...
        for i, name in enumerate(group_names):
            if i < len(groups):
                group_stats.append({"group": str(name), "n": len(groups[i]), "median": round(groups[i].median(), 4)})
